...
This website is fully optimized for free hosting on GitHub Pages.

### 1. Configure the Repository
Create a new GitHub Repository (e.g., `chic`).

### 2. Configure relative paths
The project uses `base: './'` in `vite.config.js`. This guarantees all scripts, stylesheets, and images load successfully, even when hosted on a repository subdirectory subpath (like `https://<username>.github.io/chic/`).
...
git init
git add .
git commit -m "Initialize Chic Showcase Website"
git branch -M main
git remote add origin https://github.com/YOUR_USERNAME/YOUR_REPO_NAME.git
git push -u origin main
```

### 4. Setup Automated Deployment (GitHub Actions)
Create a file named `.github/workflows/deploy.yml` with the following contents to auto-deploy on every push:
```yaml
...
Alternatively, you can build locally via `npm run build` and push the contents of the `/dist` folder directly to a `gh-pages` branch.
